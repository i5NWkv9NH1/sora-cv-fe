  ...
  <VApp>
    ...
    <VNavigationDrawer v-model="drawer" location="right" width="350" :rounded="false" temporary>
      ...
      <VContainer>
        <div>
          <div class="mb-3">
            <VLabel class="mb-2 font-weight-medium">主题设置</VLabel>
            <VMessages messages="自定义主题是明亮还是黑暗的主题，或者两者的结合" active />
          </div>
          <VRadioGroup v-model="theme">
            <VRadio v-for="item in themeMode" :key="item.value" :value="item.value"
              :true-icon="'mdi-check-circle-outline'" :color="theme === item.value ? 'primary' : 'default'">
              <template #label>
                <VIcon start>{{ item.icon || 'mdi-heart' }}</VIcon>
                {{ item.label }}
              </template>
            </VRadio>
          </VRadioGroup>
          <div class="mb-3">
            <VLabel class="mb-2 font-weight-medium">密度设置</VLabel>
            <VMessages messages="自定义主题是明亮还是黑暗的主题，或者两者的结合" active />
          </div>
          ...
          <VRadioGroup v-model="size">
            <VRadio v-for="(item) in sizeMode" :key="item.label" :value="item.value"
